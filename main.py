...
Reconecta la inferencia al modelo (ONNX), centraliza la configuración y registra cada ciclo con
logging estructurado (consola + archivo en logs/). La captura de audio está aislada en
...
def listen_for_bite(recorder, audio_cfg):
    ...
    return match_audio(recording, audio_cfg.fs, audio_cfg.references, audio_cfg)
...
def main():
    ...
    roi = CFG.roi
    log.info(
        "ROI=%s | conf=%.2f | loot=%s | escucha=%ds",
        roi.as_mss(), CFG.detector.conf_threshold, CFG.input.loot_button, CFG.audio.listen_window,
    )
    ...
    try:
        while True:
            ...
            while time.time() - inicio < CFG.audio.listen_window:
                matched, scores = listen_for_bite(recorder, CFG.audio)
                log.debug(
                    "ciclo %d: scores audio=%s",
                    ciclo, {k: round(v, 4) for k, v in scores.items()},
                )
                if matched:
                    sonido_detectado = True
                    log.info(
                        "ciclo %d: mordida por audio scores=%s",
                        ciclo, {k: round(v, 3) for k, v in scores.items()},
                    )
